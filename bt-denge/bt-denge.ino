...
//PID
//move(int leftSpeed, int rightSpeed, int minAbsSpeed)

#if MANUAL_TUNING
  double kp , ki, kd;
  double prevKp, prevKi, prevKd;
#endif
double originalSetpoint = 194.5; //174.29;   original value-rjinal değer değiştirebilirsiniz
double setpoint = originalSetpoint;
double movingAngleOffset = 0.3; //  denge hassasiyeti için
double input, output;
...
LMotorController motorController(ENA, IN1, IN2, ENB, IN3, IN4, 0.6, 1); // 0.6 ve 1 değerleri sağ ve sol motor için hız oranı
...
// join I2C bus (I2Cdev library doesn't do this automatically)
    #if I2CDEV_IMPLEMENTATION == I2CDEV_ARDUINO_WIRE
        Wire.begin();
        TWBR = 24; // 400kHz I2C clock (200kHz if CPU is 8MHz)//24
...
{   
 ...
 {  
  ...
  if(SerialData=='F')
   {setpoint = originalSetpoint +35;  
   }
  else if(SerialData=='B')
   {setpoint = originalSetpoint - 55;
   }
...
{
    int potKp = 90;  // PID değerleri değiştirebilirsiniz veya potansiyometre kullanarak en uygun değeri kendiniz bulabilirsiniz
    int potKi = 190;
    int potKd = 9;
